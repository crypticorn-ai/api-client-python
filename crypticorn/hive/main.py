...
import pandas as pd
import requests
from crypticorn.hive.utils import DataInfo, ModelEvaluation, SingleModel, download_file
...
class HiveClient:
    ...
    def __init__(
        self,
        base_url="https://api.crypticorn.com",
        api_key: str = None,
        jwt: str = None,
    ):
        """@private
    ...
    def evaluate_model(
        self, model_id: int, data: Any, version: float = None
    ) -> ModelEvaluation:
        """
        ...
        if isinstance(data, pd.DataFrame):
            ...
            if data.endswith(".feather"):
                ...
                raise ValueError(
                    "Unsupported file format. Use .feather, .parquet, or pd.Dataframe."
                )
        else:
            raise ValueError(
                "Unsupported data format. Pass a pd.DataFrame or a valid file path."
            )
    ...
    def download_data(
        self, model_id: int, version: float = None, feature_size: str = None
    ) -> int:
        """
        ...
        download_file(
            url=data["y_train"],
            dest_path=f"{base_path}y_train_{data['target']}.feather",
        )
        download_file(
            url=data["X_test"],
            dest_path=f"{base_path}X_test_{data['feature_size']}.feather",
        )
        download_file(
            url=data["X_train"],
            dest_path=f"{base_path}X_train_{data['feature_size']}.feather",
        )
        return 200
