...
class PaginationParams(BaseModel, Generic[T]):
    ...
    order: Literal["asc", "desc"] = Field(default="asc", description="The order to sort by")
    ...
    def validate(self):
        ...
        if not args or not issubclass(args[0], BaseModel):
            raise TypeError("PaginationParams must be used with a Pydantic BaseModel as a generic parameter")
        if self.sort:
            # check if the sort field is valid
            model: Type[BaseModel] = args[0]
            if self.sort and self.sort not in model.model_fields:
                raise ValueError(f"Invalid sort field: '{self.sort}' — must be one of: {list(model.model_fields)}")
        return self
